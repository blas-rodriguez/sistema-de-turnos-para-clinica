...
require_once("generar-grafico-de-tarta3.php"); 
include("conexion.php");
	
	    $sql3="SELECT * from profesionales where codigo=".$sp;
			...
			{				
				...
				{
					$combobit3 .=$row3['profesional']; //concatenamos el los options para luego ser insertado en el HTML
				}
			}
...
<body>
<h1 align="center">LISTADO DE TURNOS<BR> PROFESIONAL:'.$combobit3.'<BR> DESDE: '. $desde.' HASTA: '.$hasta.' </h1><br>
<center>
...
//$resultado=mysql_query("SELECT A.id, B.id, A.titulo, A.texto FROM $tabla A, $tabla2 B WHERE A.id=B.id IN (SELECT id, nombre, apellidos FROM AUTORES WHERE nombre=$nombre AND apellidos=$apellidos)");
        $consulta=mysqli_query($conexion,"SELECT * from turnos  WHERE fecha BETWEEN '".$desde1."' AND '".$hasta1."' and codmed='".$sp."'");
        
		$sql1="SELECT * from turnos  WHERE fecha BETWEEN '".$desde1."' AND '".$hasta1."' and codmed='".$sp."'";
