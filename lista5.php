...
require_once("generar-grafico-de-tarta5.php"); 
...
<body>
<h1 align="center">LISTADO DE TURNOS<BR> ELIMINADOS<BR> DESDE: '. $desde.' HASTA: '.$hasta.' </h1><br>
<center>
...
<div align="center">
    <table width="100%" border="1">
      <tr>
		...
		<td bgcolor="#0099FF"><strong>FECHA BAJA</strong></td>  
...
//$resultado=mysql_query("SELECT A.id, B.id, A.titulo, A.texto FROM $tabla A, $tabla2 B WHERE A.id=B.id IN (SELECT id, nombre, apellidos FROM AUTORES WHERE nombre=$nombre AND apellidos=$apellidos)");
        $consulta=mysqli_query($conexion,"SELECT * from eliminados  WHERE fecha BETWEEN '".$desde1."' AND '".$hasta1."' ");
        
		$sql1="SELECT * from eliminados  WHERE fecha BETWEEN '".$desde1."' AND '".$hasta1."' ";
...
$codigoHTML.='
	  ...
	  <tr>
		...
		<td><center>'.$dato['fechabaja'].'</center></td>
      </tr>';
      } 
$codigoHTML.='
    </table></br></br>
