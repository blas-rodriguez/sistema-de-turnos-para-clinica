 ...
 //   $desde1 = $date->format('Y-m-d');
  ...
  //  $date1 = new DateTime($cod2);
   // $hasta1 = $date1->format('Y-m-d');
	...
	//PADRON TOTAL
	 //$consulta=mysqli_query($conexion,"SELECT * FROM legajo ");
       // while($dato= mysqli_fetch_array($consulta)){
		$sql11="SELECT * from turnos  WHERE fecha BETWEEN '".$desde1."' AND '".$hasta1."' and os='".$sp."'";
		$result11 = $conexion->query($sql11);
		$combobit11 = mysqli_num_rows($result11);
		$num1 = (int)$combobit11; 
		//$num1 = 50; 
	//CANTIDAD DE ACTIVOS
	    $sql22="SELECT * from turnos  WHERE  fecha BETWEEN '".$desde1."' AND '".$hasta1."' and os='".$sp."' and estado=2";
		$result22 = $conexion->query($sql22);
		$combobit22 = mysqli_num_rows($result22); 
		$num2 = (int)$combobit22;
		//$num2 = 50;
		//CANTIDAD DE BAJAS
	    $sql33="SELECT * from turnos  WHERE  fecha BETWEEN '".$desde1."' AND '".$hasta1."' and os='".$sp."' and not estado=2";
	...
	//Definimos el titulo 
	$grafico->title->Set("OBRAS SOCIALES");
