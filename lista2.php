...
require_once("generar-grafico-de-tarta2.php"); 
include("conexion.php");
	
	    $sql3="SELECT * from especialidades where codigo=".$sp;
			...
			{				
				...
				{
					$combobit3 .=$row3['especialidad']; //concatenamos el los options para luego ser insertado en el HTML
				}
			}
...
<body>
<h1 align="center">LISTADO DE TURNOS<BR> ESPECIALIDAD:'.$combobit3.'<BR> DESDE: '. $desde.' HASTA: '.$hasta.' </h1><br>
<center>
...
<div align="center">
    <table width="100%" border="1">
	  ...
	  <td bgcolor="#0099FF"><strong>HORA</strong></td>
        <td bgcolor="#0099FF"><strong>PROFESIONAL</strong></td>
        <td bgcolor="#0099FF"><strong>PACIENTE</strong></td>  
		<td bgcolor="#0099FF"><strong>TEL</strong></td>  
		<td bgcolor="#0099FF"><strong>OBRA S.</strong></td> 
...
//$resultado=mysql_query("SELECT A.id, B.id, A.titulo, A.texto FROM $tabla A, $tabla2 B WHERE A.id=B.id IN (SELECT id, nombre, apellidos FROM AUTORES WHERE nombre=$nombre AND apellidos=$apellidos)");
        $consulta=mysqli_query($conexion,"SELECT * from turnos  WHERE fecha BETWEEN '".$desde1."' AND '".$hasta1."' and codesp='".$sp."'");
        
		$sql1="SELECT * from turnos  WHERE fecha BETWEEN '".$desde1."' AND '".$hasta1."' and codesp='".$sp."'";
